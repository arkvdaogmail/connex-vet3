...
if not MNEMONIC_PHRASE:
    ...
    try:
        ...
        SENDER_ADDRESS = bip44_addr.PublicKey().ToAddress()
...
def send_vechain_transaction(data_to_store_on_chain: str):
    ...
    try:
        # 1. Get latest block for chainTag and blockRef
        response = requests.get(f"{NODE_URL}/blocks/best")
        response.raise_for_status()
        latest_block = response.json()
        chain_tag = int(latest_block['chainTag'], 16)
        block_ref = latest_block['id'][2:18]  # 8 bytes (16 hex chars) after '0x'
        ...
        # 3. Estimate gas (optional, here we use a fixed value for simplicity)
        gas = 80000
        ...
        # 5. Sign transaction using thor-devkit
